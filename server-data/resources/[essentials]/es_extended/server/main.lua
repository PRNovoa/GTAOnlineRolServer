...
function loadESXPlayer(identifier, playerId)
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT accounts, job, job_grade, job2, job2_grade, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier', { -- Secondary/Mafia jobs: job2, job2_grade,
			['@identifier'] = identifier
		}, function(result)
			local job, grade, jobObject, gradeObject = result[1].job, tostring(result[1].job_grade)
			local job2, grade2, job2Object, grade2Object = result[1].job2, tostring(result[1].job2_grade) -- Secondary/Mafia jobs
	...
	Async.parallel(tasks, function(results)
		local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.weight, userData.job, userData.job2, userData.loadout, userData.playerName, userData.coords) -- Secondary/Mafia jobs: userData.job2
		...
		xPlayer.triggerEvent('esx:playerLoaded', {
			...
			job = xPlayer.getJob(),
			job2 = xPlayer.getJob2(), -- Secondary/Mafia jobs
