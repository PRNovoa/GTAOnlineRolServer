...
ESX.SavePlayer = function(xPlayer, cb)
	...
	table.insert(asyncTasks, function(cb2)
		MySQL.Async.execute('UPDATE users SET accounts = @accounts, job = @job, job_grade = @job_grade, job2 = @job2, job2_grade = @job2_grade, `group` = @group, loadout = @loadout, position = @position, inventory = @inventory WHERE identifier = @identifier', { -- Secondary/Mafia jobs:  job2 = @job2, job2_grade = @job2_grade,
			['@accounts'] = json.encode(xPlayer.getAccounts(true)),
			['@job'] = xPlayer.job.name,
			['@job_grade'] = xPlayer.job.grade,
			['@job2'] = xPlayer.job2.name,		-- Secondary/Mafia jobs
			['@job2_grade'] = xPlayer.job2.grade, -- Secondary/Mafia jobs
