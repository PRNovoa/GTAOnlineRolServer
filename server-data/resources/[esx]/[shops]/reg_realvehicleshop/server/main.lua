...
Citizen.CreateThread(function()
	...
	if char > 8 then
		print(('[reg_realvehicleshop] [^3WARNING^7] Plate character count reached, %s/8 characters!'):format(char))
	end
end)
...
MySQL.ready(function()
	MySQL.Async.fetchAll('SELECT * FROM vehiclereal_categories', {}, function(_categories)
		categories = _categories

		MySQL.Async.fetchAll('SELECT * FROM vehiclesreal ORDER BY `category` ASC, `name` ASC', {}, function(_vehicles)
			vehicles = _vehicles
			...
			TriggerClientEvent('reg_realvehicleshop:sendCategories', -1, categories)
			TriggerClientEvent('reg_realvehicleshop:sendVehicles', -1, vehicles)
		end)
	end)
end)
...
RegisterNetEvent('reg_realvehicleshop:setVehicleOwnedPlayerId')
AddEventHandler('reg_realvehicleshop:setVehicleOwnedPlayerId', function(playerId, vehicleProps, model, label)
...
ESX.RegisterServerCallback('reg_realvehicleshop:getSoldVehicles', function(source, cb)
	MySQL.Async.fetchAll('SELECT client, model, plate, soldby, date FROM vehicle_sold', {}, function(result)
		cb(result)
	end)
end)

RegisterNetEvent('reg_realvehicleshop:rentVehicle')
AddEventHandler('reg_realvehicleshop:rentVehicle', function(vehicle, plate, rentPrice, playerId)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(playerId)
...
RegisterNetEvent('reg_realvehicleshop:getStockItem')
AddEventHandler('reg_realvehicleshop:getStockItem', function(itemName, count)
...
RegisterNetEvent('reg_realvehicleshop:putStockItems')
AddEventHandler('reg_realvehicleshop:putStockItems', function(itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('reg_realvehicleshop:getCategories', function(source, cb)
	cb(categories)
end)

ESX.RegisterServerCallback('reg_realvehicleshop:getVehicles', function(source, cb)
	cb(vehicles)
end)

ESX.RegisterServerCallback('reg_realvehicleshop:buyVehicle', function(source, cb, model, plate)
...
ESX.RegisterServerCallback('reg_realvehicleshop:getCommercialVehicles', function(source, cb)
	MySQL.Async.fetchAll('SELECT price, vehicle FROM cardealer_vehicles ORDER BY vehicle ASC', {}, function(result)
		cb(result)
	end)
end)

ESX.RegisterServerCallback('reg_realvehicleshop:buyCarDealerVehicle', function(source, cb, model)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('reg_realvehicleshop:returnProvider')
AddEventHandler('reg_realvehicleshop:returnProvider', function(vehicleModel)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if xPlayer.job.name == 'cardealer' then
		MySQL.Async.fetchAll('SELECT id, price FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
			...
			if result[1] then
				...
				print(('[reg_realvehicleshop] [^3WARNING^7] %s attempted selling an invalid vehicle!'):format(xPlayer.identifier))
			end
		end)
	end
end)

ESX.RegisterServerCallback('reg_realvehicleshop:getRentedVehicles', function(source, cb)
...
ESX.RegisterServerCallback('reg_realvehicleshop:giveBackVehicle', function(source, cb, plate)
...
ESX.RegisterServerCallback('reg_realvehicleshop:resellVehicle', function(source, cb, plate, model)
	...
	if xPlayer.job.name == 'cardealer' then
		...
		if not resellPrice then
			print(('[reg_realvehicleshop] [^3WARNING^7] %s attempted to sell an unknown vehicle!'):format(xPlayer.identifier))
			cb(false)
		else
						...
						if result[1] then -- does the owner match?
							...
							if vehicle.model == model then
								if vehicle.plate == plate then
									...
									print(('[reg_realvehicleshop] [^3WARNING^7] %s attempted to sell an vehicle with plate mismatch!'):format(xPlayer.identifier))
									cb(false)
								end
							else
								print(('[reg_realvehicleshop] [^3WARNING^7] %s attempted to sell an vehicle with model mismatch!'):format(xPlayer.identifier))
								cb(false)
							end
						end
					end)
...
ESX.RegisterServerCallback('reg_realvehicleshop:getStockItems', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		cb(inventory.items)
	end)
end)

ESX.RegisterServerCallback('reg_realvehicleshop:getPlayerInventory', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('reg_realvehicleshop:isPlateTaken', function(source, cb, plate)
...
ESX.RegisterServerCallback('reg_realvehicleshop:retrieveJobVehicles', function(source, cb, type)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('reg_realvehicleshop:setJobVehicleState')
AddEventHandler('reg_realvehicleshop:setJobVehicleState', function(plate, state)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('[reg_realvehicleshop] [^3WARNING^7] %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)
...
function PayRent(d, h, m)
	...
	print('[reg_realvehicleshop] [^2INFO^7] Paying rent cron job started')
	...
	MySQL.Async.fetchAll('SELECT owner, rent_price, plate FROM rented_vehicles', {}, function(result)
		...
		print(('[reg_realvehicleshop] [^2INFO^7] Paying rent cron job took %s seconds'):format(elapsedTime))
	end)
end
