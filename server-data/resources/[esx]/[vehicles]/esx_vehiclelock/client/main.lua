	...
	ESX.TriggerServerCallback('esx_vehiclelock:requestPlayerCars', function(isOwnedVehicle)
		...
		if isOwnedVehicle then
			...
			if lockStatus == 1 then -- unlocked
				...
				ESX.ShowAdvancedNotification('Control de vehículo', 'Puertas', '~r~Coche cerrado', mugshotStr, 0)
			...
			elseif lockStatus == 2 then -- locked
				...
				local mugshot, mugshotStr = ESX.Game.GetPedMugshot(PlayerPedId())
				ESX.ShowAdvancedNotification('Control de vehículo', 'Puertas', '~g~Coche abierto', mugshotStr, 0)
				UnregisterPedheadshot(mugshot)
			end
		end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 303) and IsInputDisabled(0) then -- U
			ToggleVehicleLock()
			Citizen.Wait(300)
