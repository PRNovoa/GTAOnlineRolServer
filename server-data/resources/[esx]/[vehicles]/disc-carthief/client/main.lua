...
function SpawnCar()
    ESX.TriggerServerCallback('disc-carthief:isActive', function(isActive, isCooldownActive)
        if isCooldownActive == 1 then
            ESX.ShowNotification("~r~Cooldown is active!")
        elseif isActive == 0 then
            ESX.TriggerServerCallback('disc-carthief:anycops', function(anycops)
                if anycops >= Config.CopsRequired then
                    ...
                    DecorSetFloat(car, "_FUEL_LEVEL", GetVehicleFuelLevel(car))
                    ...
                    ESX.ShowNotification("~r~Not enough cops in town!")
                end
            end)
        else
            ESX.ShowNotification("~r~There is already a car robbery in progress!")
        end
    end)
end
...
function FinishDelivery()
    if (GetVehiclePedIsIn(GetPlayerPed(-1), false) == car) and GetEntitySpeed(car) < 3 then
        ...
        ESX.ShowNotification("~g~You finished the robbery, Here is your payment! You earned $" .. finalpayment)
        ...
        ESX.ShowNotification("~r~You have to use the car that was provided for you and you must come to a full stop.")
    end
end
...
function AbortDelivery()
    ...
    if isTaken == 1 and isDelivered == 0 then
        ...
        ESX.ShowNotification("~r~Mission failed!")
...
function Buy()
    ESX.TriggerServerCallback('disc-carthief:buyJammer', function(bought)
        if bought == 1 then
            ESX.ShowNotification("You bought a Jammer!")
        elseif bought == -1 then
            ESX.ShowNotification("~r~You need $" .. Config.JammerPrice .. " (Dirty) to buy a jammer")
        elseif bought == 0 then
            ESX.ShowNotification("~r~You already have a jammer!")
        end
    end)
end
...
AddEventHandler('disc-carthief:jammerActive', function()
    if isTaken == 1 and isDelivered == 0 and (GetVehiclePedIsIn(GetPlayerPed(-1), false) == car) then
        isJammerActive = 1
        ESX.ShowNotification("Activating Jammer!")
...
Citizen.CreateThread(function()
    while true do
        ...
        if isTaken == 1 and isDelivered == 0 and not (GetVehiclePedIsIn(GetPlayerPed(-1), false) == car) then
            ESX.ShowNotification("~r~You have 1 minute to get back in the car")
            Wait(50000)
            if isTaken == 1 and isDelivered == 0 and not (GetVehiclePedIsIn(GetPlayerPed(-1), false) == car) then
                ESX.ShowNotification("~r~You have 10 seconds to get back in the car")
                Wait(10000)
                AbortDelivery()
            end
        end
...
AddEventHandler('disc-carthief:activatedJammerCops', function()
    ESX.ShowNotification("~r~Criminals have activated their signal jammer! Beware!")
end)
