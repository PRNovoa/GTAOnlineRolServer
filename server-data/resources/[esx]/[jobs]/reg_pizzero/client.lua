...
local blips = {
  {title="Pizzeria Luigi", colour=51, id=103, x = -1285.73, y = -1387.15, z = 3.44}, --Configuration du point sur la carte
}

local coefflouze = 0.15 --Coeficient multiplicateur qui en fonction de la distance definit la paie
      ...
      if GetDistanceBetweenCoords(pizzeria.x, pizzeria.y, pizzeria.z, GetEntityCoords(GetPlayerPed(-1),true)) < 1.5 then
        ...
        if IsControlJustPressed(1,38) then
          if ESX.PlayerData.job and ESX.PlayerData.job.name == "pizza" then
            ...
            paie = math.floor(distance * coefflouze)
            ...
            TriggerEvent("pk_notifications:SendNotification", {
              text = "No eres pizzero, primero date de alta",
              --type = "info",
              queue = "global",
              timeout = 4000,
              -- layout = "bottomRight"
