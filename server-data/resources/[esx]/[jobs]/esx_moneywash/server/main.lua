...
AddEventHandler('esx_moneywash:washMoney', function(amount, zone)
	...
	if enableTimer == true then
		...
		if amount > 0 and xPlayer.getAccount('black_money').money >= amount and amount % 5000 == 0 then
			...
			do
				...
				TriggerClientEvent('esx:showNotification', xPlayer.source, _U('you_have_received') .. ESX.Math.GroupDigits(5000) .. _U('clean_money'))
				washedTotal = washedTotal - 5000
