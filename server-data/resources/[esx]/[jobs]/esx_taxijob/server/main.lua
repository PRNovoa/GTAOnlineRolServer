...
AddEventHandler('esx_taxijob:success', function()
	...
	local percentageWorker = 30
	local percentageSociety = 70
	...
	if xPlayer.job.name == 'taxi' then
		if not lastPlayerSuccess[source] or timeNow - lastPlayerSuccess[source] > 5 then
			...
			TriggerEvent('esx_addonaccount:getSharedAccount', 'society_taxi', function(account)
				if account then
					if Config.CustomPercentages then
						percentageWorker = Config.PercentageWorker
						percentageSociety = Config.PercentageSociety
					end
					local playerMoney  = ESX.Math.Round(total / 100 * percentageWorker)
					local societyMoney = ESX.Math.Round(total / 100 * percentageSociety)
