...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(1, Keys["G"]) and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == Config.JobName then
			...
			if Onjob then
				...
				if IsPedInAnyVehicle(playerPed, false) and IsVehicleModel(GetVehiclePedIsIn(playerPed, false), GetHashKey("dinghy2")) then
					StartNPCJob()
					Onjob = true
				else
