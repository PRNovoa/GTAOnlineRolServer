...
$(function() {
    window.addEventListener('message', function(event) {
        ...
        if (event.data.note_deleted) {
            ...
            noteMessage.empty();
            noteMessage.text('Note surpprimé');
            setTimeout(function() {
                noteMessage.empty();
            }, 2000);
        ...
        if (event.data.note_not_deleted) {
            noteMessage = $('.note-message');
            noteMessage.empty();
            noteMessage.text('Suppression de la note echoué');
            setTimeout(function() {
                noteMessage.empty();
            }, 2000);
        }
        ...
        if (event.data.bolo_deleted) {
            ...
            boloMessage.empty();
            boloMessage.text('La recherché a été retiré.');
            setTimeout(function() {
                boloMessage.empty();
            }, 2000);
...
function showExtraUserData(user) {
    ...
    $('.firstname-label').text('Prénom');
    $('.firstname').text(user.firstname);

    $('.lastname-label').text('Nom');
    $('.lastname').text(user.lastname);

    $('.sex-label').text('Genre');
    $('.sex').text(user.sex);

    $('.dob-label').text('Date de naissance');
    $('.dob').text(user.dateofbirth);

    $('.height-label').text('Taille');
    $('.height').text(user.height);
    $('.job-label').text('Employé dans');
    $('.job').text(user.job);

    $('.jail-label').text('Prison');
    $('.jail').text((user.jail ? 'Oui' : 'Non'));
