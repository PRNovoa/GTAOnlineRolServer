...
AddEventHandler('esx_ambulancejob:notif', function()
	...
	ReaFaite = true
	ESX.ShowAdvancedNotification('Info Cruz Roja', 'Central Cruz Roja', 'Reanimación terminada. \n~g~1500€~w~ añadidos a los fondos públicos.\n~g~'..Nombreinter..' intervención en progreso.', 'CHAR_MP_MORS_MUTUAL', 3)
end)
...
function RemoveItemsAfterRPDeath()
	...
	Citizen.CreateThread(function()
		...
		PlaySoundFrontend(-1, "1st_Person_Transition", "PLAYER_SWITCH_CUSTOM_SOUNDSET", 0)
		ESX.ShowAdvancedNotification('Reanimación X', 'Resucitación Unidad X', 'Has sido reanimado por la unidad especial X.', 'CHAR_CALL911', 1)
		local ped = GetPlayerPed(PlayerId())
		local coords = GetEntityCoords(ped, false)
		local name = GetPlayerName(PlayerId())
		local x, y, z = table.unpack(GetEntityCoords(ped, false))
		TriggerServerEvent('esx_ambulance:NotificationBlipsX', x, y, z, name)
		Citizen.Wait(10*1000) -- Effets de la réanmation pendant 1 minute ( 60 seconde )
		Normal()
	end)
end
