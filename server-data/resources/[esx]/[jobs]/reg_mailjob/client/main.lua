        ...
        if CurrentAction ~= nil then
            ...
            if IsControlJustReleased(0, 38) and IsJobTrucker() then -- Touche E
				...
				if CurrentAction == 'vehiclespawner' then
					if hasVehicle then
						ESX.ShowNotification("~r~Ya has sacado un vehículo")
					else
						VehiculeSpawner()
						hasVehicle = true
						Citizen.SetTimeout(1000, function()
							SetVehicleFuelLevel(GetVehiclePedIsIn(PlayerPedId()), 100.0)
    						DecorSetFloat(GetVehiclePedIsIn(PlayerPedId()), "_FUEL_LEVEL", 100.0)
						end)
					end
                end
                ...
                if CurrentAction == 'vehicledeleter' then
					...
					if hasVehicle then
						...
						ESX.ShowNotification("~r~No tienes ningún vehículo que devolver")
					end
                end
...
-- CLIENT SIDED ONLY
local vector =  vector3(69.03, 126.08, 80.19) 
default = {
    xyz = vector, -- vectors work too
    text={
        content="Bienvenido a ~y~Correos.",
        rgb={255 , 255, 255},
        textOutline=true,
        scaleMultiplier=1,
        font=0
...
-- CLIENT SIDED ONLY
local vector =  vector3(69.03, 126.08, 80.09) 
default = {
    xyz = vector, -- vectors work too
    text={
        content="~w~Una vez saques la ~b~furgoneta ",
        rgb={255 , 255, 255},
        textOutline=true,
        scaleMultiplier=1,
        font=0
...
-- CLIENT SIDED ONLY
local vector =  vector3(69.03, 126.08, 79.99) 
default = {
    xyz = vector, -- vectors work too
    text={
        content="tienes que pulsar ~r~F6 ~w~para poder ~g~empezar a repartir~w~. ",
        rgb={255 , 255, 255},
        textOutline=true,
        scaleMultiplier=1,
        font=0
