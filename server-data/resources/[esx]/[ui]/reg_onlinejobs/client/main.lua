...
RegisterNetEvent('OnlineJobs:getPlayerId')
AddEventHandler('OnlineJobs:getPlayerId', function(connectedPlayers)
	SendNUIMessage({
		action  = 'startID',
...
function UpdatePlayerTable(connectedPlayers)
	...
	local ems, police, guardiacivil, taxi, mechanic, players = 0, 0, 0, 0, 0, 0, 0 -- Añadir aqui los trabajos
 ...
 -- Apara añadir mas trabajos al contador copie uno y pegalo, cambia el nombre del trabajo
		...
		elseif v.job == 'police' then
			police = police + 1
		elseif v.job == 'guardiacivil' then
			guardiacivil = guardiacivil + 1
	...
	SendNUIMessage({
		...
		jobs   = {ems = ems, police = police, guardiacivil = guardiacivil, taxi = taxi, mechanic = mechanic, player_count = players} -- Esta linea es para que se actualice el contador de trabajos
	})
end
