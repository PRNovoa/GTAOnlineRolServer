...
Citizen.CreateThread(function()
	WarMenu.CreateMenu('test', 'Alertas Policiales')
	WarMenu.CreateSubMenu('closeMenu', 'test', 'Estas seguro?')   
	while true do
		if WarMenu.IsMenuOpened('test') then
			if WarMenu.MenuButton('~b~Alerta azul', 'closeMenu' )  then

ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'SystemAlert',
    {
        title = ('Ingresar Anuncio'),
    }, function(data, menu)
        local length = string.len(data.value)
      
        if data.value == nil  or length < 2 or length > 200 then 
            ESX.ShowNotification("Error")
        else 
str = data.value
TriggerServerEvent("test:find", str)
        end
          menu.close()
...
elseif WarMenu.MenuButton('~g~Alerta verde', 'closeMenu') then
ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'SystemAlert',
    {
        title = ('Ingresar Anuncio'),
    }, function(data, menu)
...
TriggerServerEvent("RD:JmColor1", str)
            end
            menu.close()
    end, function(data, menu)
        menu.close()
...
elseif WarMenu.MenuButton('~y~Alerta amarilla', 'closeMenu') then
...
str = data.value
TriggerServerEvent("RD:JmColor2", str)
            end
            menu.close()
    end, function(data, menu)
        menu.close()
...
elseif WarMenu.MenuButton('~o~Alerta naranja', 'closeMenu') then
ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'SystemAlert',
    {
        title = ('Ingresar Anuncio'),
    }, function(data, menu)
...
str = data.value
TriggerServerEvent("RD:JmColor3", str)
           end
            menu.close()
    end, function(data, menu)
        menu.close()
...
elseif WarMenu.MenuButton('~r~Alerta roja', 'closeMenu') then
ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'SystemAlert',
    {
        title = ('Ingresar Anuncio'),
    }, function(data, menu)
...
str = data.value
TriggerServerEvent("RD:JmColor4", str)
            end
            menu.close()
    end, function(data, menu)
        menu.close()
...
elseif WarMenu.MenuButton('~r~Desactivar Alertas', 'closeMenu') then
TriggerServerEvent("DS:desac")
elseif WarMenu.MenuButton('~c~Exit', 'closeMenu') then
end
			WarMenu.Display()			
		elseif IsControlJustPressed(1, 51) and tes then 
			tes = false
...
function WarMenu.CreateMenu(id, title)
	...
	menus[id].subTitle = 'Sistema de Alertas'
...
if(Vdist(pos.x, pos.y, pos.z, 446.34, -973.38, 30.69)  < 0.4) and not tes  and not cuad then
DisplayHelpText('~g~Presiona ~g~~INPUT_CONTEXT~ Para cambiar el Nivel de Alerta')
end
if(Vdist(pos.x, pos.y, pos.z, 446.34, -973.38, 30.69)  < 0.4) and IsControlJustPressed(1, 51) then
tes = true
else 
end
end
